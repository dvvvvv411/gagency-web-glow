...
const BenefitsSlider = () => {
  ...
  return <section className="py-16 bg-gradient-to-br from-gray-50 via-white to-gray-50 relative overflow-hidden">
      {/* Background Pattern */}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative">
        ...
        <div className="text-center mb-12">
          <h2 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">
            Warum <span className="bg-gradient-to-r from-primary-600 to-blue-600 bg-clip-text text-transparent">INGENIO?</span>
          </h2>
          <p className="text-lg text-gray-600 max-w-2xl mx-auto">Entdecken Sie, was INGENIO zum perfekten Sprungbrett für Ihre Karriere macht</p>
        </div>
        ...
        <div className="relative bg-white rounded-3xl shadow-2xl overflow-hidden max-w-4xl mx-auto">
          ...
          <div className="absolute top-0 left-0 w-full h-1 bg-gray-100">
            <div className={`h-full bg-gradient-to-r ${currentBenefit.gradient} transition-all duration-4000 ease-linear`} style={{
          ...
          <div className="grid lg:grid-cols-2 gap-8 p-8 lg:p-12">
            ...
            <div className="flex flex-col justify-center space-y-6">
              ...
              <div className="flex items-center space-x-4 pt-4">
                <Button onClick={prevBenefit} variant="outline" size="icon" className="rounded-full">
                  <ChevronLeft size={20} />
                </Button>

                <Button onClick={toggleAutoplay} variant="outline" size="icon" className="rounded-full">
                  {isPlaying ? <Pause size={18} /> : <Play size={18} />}
                </Button>

                <Button onClick={nextBenefit} variant="outline" size="icon" className="rounded-full">
                  <ChevronRight size={20} />
                </Button>

                <div className="flex space-x-2 ml-4">
                  {benefits.map((_, index) => <button key={index} onClick={() => setCurrentIndex(index)} className={`w-2 h-2 rounded-full transition-all duration-300 ${index === currentIndex ? `bg-gradient-to-r ${currentBenefit.gradient}` : 'bg-gray-300 hover:bg-gray-400'}`} />)}
                </div>
              </div>
            </div>
            ...
            <div className="relative flex items-center justify-center">
              ...
              <div className="absolute bottom-8 left-8 w-3 h-3 bg-white rounded-full shadow-lg animate-bounce opacity-40" style={{
              animationDelay: '1s'
            }}></div>
              <div className="absolute top-1/2 left-8 w-2 h-2 bg-white rounded-full shadow-lg animate-ping opacity-30" style={{
              animationDelay: '2s'
            }}></div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-12 max-w-3xl mx-auto">
          ...
          value: '4.9/5'
        }].map((stat, index) => <div key={stat.label} className="text-center p-4 bg-white rounded-xl shadow-sm">
